...
def connect_db():
    return psycopg2.connect(
        dbname="postgres", user="postgres", password="NAME_Shokxn", host="localhost"
    )
...
def update_data_console():
    ...
    try:
        ...
        if new_first_name and new_last_name:
            ...
            cursor.execute("CALL insert_or_update_user(%s, (SELECT last_name FROM phonebook WHERE phone = %s), %s)", (new_first_name, phone, phone))
        elif new_last_name:  # Обновляем только фамилию
            cursor.execute("CALL insert_or_update_user((SELECT first_name FROM phonebook WHERE phone = %s), %s, %s)", (phone, new_last_name, phone))
        else:
